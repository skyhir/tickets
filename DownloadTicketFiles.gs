function downloadReloadFilesOnly() {
  ...
  const LINK_COL = 22;   // V
  const FILTER_COL = 16; // P

  const data = sheet.getRange(startRow, 1, lastRow - startRow + 1, LINK_COL).getValues();

  const folder = DriveApp.createFolder("Reload Files " + new Date().toISOString());
  ...
  data.forEach((row, i) => {
    ...
    if (!link) return;
    if (String(filterValue).trim() !== "Reload") return;
